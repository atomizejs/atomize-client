...
(function () {
    ...
    function ProxyInjector(ast, atomize, ignore) {
        this.ast = ast;
        this.treeWalker = new TreeWalker(ast, this);
        if (undefined !== ignore) {
            this.ignore = ignore;
    ...
    ProxyInjector.prototype = {
        ...
        ignored: function (name) {
            return -1 !== this.ignore.indexOf(name);
        },
        ...
        AssignmentExpression: function (tw, parent, obj, name) {
            ...
            if (obj.left.type === 'PropertyAccess') {
                ...
                tw.invoke(function () {
                    ...
                    if ("=" !== obj.operator) {
                        rhs = self.atomize + ".access(" + base + ", " + setter + ") " +
                            obj.operator.substring(0, obj.operator.length - 1) + " " + rhs;
                    }
        ...
        ForInStatement: function (tw, parent, obj, name) {
            ...
            tw.invoke(function () {
                var body, collection;
                collection = new PrettyPrinter(obj.collection).print();

                if (self.ignored(collection)) {
                    return;
                }

                body = self.atomize + ".enumerate(" + collection + ")";
                obj.collection = parser.parse(body).elements[0];
            });
            tw.walk('iterator');
            tw.walk('collection');
        },
        ...
        BinaryExpression: function (tw, parent, obj, name) {
            ...
            tw.invoke(function () {
                if ("in" === obj.operator) {
                    var body, container;
                    container = new PrettyPrinter(obj.right).print();
                    if (self.ignored(container)) {
                        return;
                    }
                    body = self.atomize + ".has(" + container + ", " +
                        (new PrettyPrinter(obj.left).print()) + ")";
                    parent[name] = parser.parse(body).elements[0];
                }
            });
            tw.walk('right');
        ...
        UnaryExpression: function (tw, parent, obj, name) {
            ...
            deleting = "delete" === obj.operator && "PropertyAccess" === obj.expression.type;
            if (deleting) {
                tw.invoke(function () {
                    // we just want to swap the 'access' with a 'erase' call...
                    parent[name].expression.name.name = 'erase'
                    // ... and wipe out the delete UnaryExpression
                    parent[name] = parent[name].expression;
                });
            }
            tw.walk('expression');
        }
    ...
    exports.test = function () {
        var ast, pi, pp;
        ast = parser.parse("a.b['c'] = d.e = f['' + g]; for (var j in a) { console.log(j); }; console.log('b' in a);");
